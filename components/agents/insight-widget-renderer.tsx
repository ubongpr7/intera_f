...
function WidgetCard({
  ...
  return (
    <section className="rounded-[24px] border border-gray-200 bg-white p-4 shadow-[0_18px_44px_-34px_rgba(15,23,42,0.28)]">
      ...
      {fields.map((field) => {
        ...
        return (
          <label key={name} className="block">
            <span className="mb-1.5 block text-xs font-medium uppercase tracking-[0.14em] text-gray-500 dark:text-gray-400">
              {label}
            </span>
            {type === "select" ? (
              <select
                ...
                className="w-full rounded-2xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-900 outline-none ring-0 transition focus:border-blue-400 dark:border-gray-700 dark:bg-gray-950 dark:text-gray-100"
              >
...
function renderWidget(
      ...
      <WidgetCard key={`${type}-${index}`} title={title} subtitle={subtitle}>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {items.map((item, itemIndex) => (
            <div key={`${title}-${itemIndex}`} className="rounded-[20px] bg-gray-50 px-4 py-3 dark:bg-gray-950">
              <p className="text-xs font-medium uppercase tracking-[0.14em] text-gray-500 dark:text-gray-400">{asString(item.label) || `Metric ${itemIndex + 1}`}</p>
              ...
              {asString(item.detail) ? <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{asString(item.detail)}</p> : null}
            </div>
          ))}
        </div>
      </WidgetCard>
              ...
              content={
                <ChartTooltipContent
                  formatter={(value, name) => (
                    <>
                      <span className="text-gray-500 dark:text-gray-400">{String(name)}</span>
      ...
      <WidgetCard key={`${type}-${index}`} title={title} subtitle={subtitle}>
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {data.map((item, itemIndex) => (
            <div key={`${title}-${itemIndex}`} className="flex items-center justify-between rounded-2xl bg-gray-50 px-3 py-2 text-sm dark:bg-gray-950">
              <div className="flex items-center gap-2">
                <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: palette[itemIndex % palette.length] }} />
                <span className="text-gray-700 dark:text-gray-300">{asString(item[labelKey])}</span>
              </div>
      ...
      <WidgetCard key={`${type}-${index}`} title={title} subtitle={subtitle}>
        ...
        {series.length ? (
          <div className="mt-3 flex flex-wrap gap-2">
            {renderedSeries.map((item) => (
              <span key={item.key} className="inline-flex items-center gap-2 rounded-full bg-gray-50 px-3 py-1 text-xs font-medium text-gray-600 dark:bg-gray-950 dark:text-gray-300">
    ...
    return (
      <WidgetCard key={`${type}-${index}`} title={title || asString(metric.label)} subtitle={subtitle}>
        <div className="grid gap-4 md:grid-cols-[180px,1fr]">
          <div className="rounded-[20px] bg-gray-50 px-4 py-3 dark:bg-gray-950">
            <p className="text-xs font-medium uppercase tracking-[0.14em] text-gray-500 dark:text-gray-400">{asString(metric.label) || "Metric"}</p>
            <p className="mt-2 text-2xl font-semibold text-gray-950 dark:text-gray-100">
            ...
            {asString(metric.detail) ? <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{asString(metric.detail)}</p> : null}
          </div>
      ...
      <WidgetCard key={`${type}-${index}`} title={title} subtitle={subtitle}>
        <div className="space-y-2">
          {items.map((item, itemIndex) => {
            ...
            return (
            <div key={`${title}-${itemIndex}`} className="flex items-start justify-between gap-3 rounded-[20px] bg-gray-50 px-4 py-3 dark:bg-gray-950">
              <div className="flex min-w-0 gap-3">
                <span className="mt-0.5 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white text-xs font-semibold text-gray-600 dark:bg-gray-900 dark:text-gray-300">
                  {itemIndex + 1}
                </span>
                {imageUrl ? (
                ...
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-gray-950 dark:text-gray-100">{asString(item.label) || asString(item.title) || "Item"}</p>
                  {asString(item.detail) ? <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{asString(item.detail)}</p> : null}
                  {barcodeValue ? (
                    <div className="mt-2">
                      <span className="inline-flex rounded-full bg-white px-2.5 py-1 text-[11px] font-medium text-gray-600 dark:bg-gray-900 dark:text-gray-300">
      ...
      <WidgetCard key={`${type}-${index}`} title={title} subtitle={subtitle}>
        <div className="space-y-3">
          {items.map((item, itemIndex) => {
            ...
            return (
              <div key={`${title}-${itemIndex}`} className="rounded-[20px] border border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-gray-950">
                <div className="flex items-start gap-2">
                  {icon}
                  <div>
                    <p className="text-sm font-semibold text-gray-950 dark:text-gray-100">{asString(item.label) || "Risk"}</p>
          ...
          <table className="min-w-full border-separate border-spacing-y-2 text-sm">
            <thead>
              <tr>
                {normalizedColumns.map((column) => (
                  <th key={column.key} className="px-3 py-1 text-left text-xs font-semibold uppercase tracking-[0.12em] text-gray-500 dark:text-gray-400">
                    {column.label}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {rows.map((row, rowIndex) => (
                <tr key={`${title}-${rowIndex}`} className="rounded-2xl bg-gray-50 dark:bg-gray-950">
                  {normalizedColumns.map((column) => (
                    <td key={`${rowIndex}-${column.key}`} className="px-3 py-3 text-gray-700 dark:text-gray-300">
            ...
            <div key={`${title}-${eventIndex}`} className="grid grid-cols-[20px,1fr] gap-3">
              ...
              <div className="rounded-[20px] bg-gray-50 px-4 py-3 dark:bg-gray-950">
                <div className="flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                  ...
                  {asString(event.severity) ? <span className="rounded-full bg-white px-2 py-0.5 font-medium text-gray-700 dark:bg-gray-900 dark:text-gray-300">{asString(event.severity)}</span> : null}
                </div>
                <p className="mt-2 text-sm font-semibold text-gray-950 dark:text-gray-100">{asString(event.title) || asString(event.event_name) || "Event"}</p>
      ...
      <WidgetCard key={`${type}-${index}`} title={title} subtitle={subtitle}>
        <div className="space-y-3">
          {steps.map((step, stepIndex) => {
            ...
            return (
              <div key={`${title}-${stepIndex}`} className="flex items-start gap-3 rounded-[20px] bg-gray-50 px-4 py-3 dark:bg-gray-950">
                {icon}
                <div>
                  <p className="text-sm font-semibold text-gray-950 dark:text-gray-100">{asString(step.label) || `Step ${stepIndex + 1}`}</p>
                  {asString(step.detail) ? <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{asString(step.detail)}</p> : null}
                </div>
              </div>
            )
          })}
  ...
  if (type === "confirmation_card") {
    return (
      <WidgetCard key={`${type}-${index}`} title={title} subtitle={subtitle}>
        <div className="rounded-[20px] bg-gray-50 px-4 py-4 dark:bg-gray-950">
          <p className="text-sm font-semibold text-gray-950 dark:text-gray-100">{asString(widget.summary) || "Confirm this action."}</p>
          {asString(widget.risk_level) ? <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Risk level: {asString(widget.risk_level)}</p> : null}
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button
              type="button"
              className="rounded-full border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800"
              onClick={() =>
                onSend(
                  JSON.stringify(
  ...
  if (type === "entity_preview") {
    ...
    return (
      <WidgetCard key={`${type}-${index}`} title={title} subtitle={subtitle}>
        <div className="flex items-start gap-4 rounded-[20px] bg-gray-50 px-4 py-4 dark:bg-gray-950">
          {asString(entity.image_url) ? (
            ...
            <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-white text-xs font-semibold uppercase tracking-[0.14em] text-gray-500 dark:bg-gray-900 dark:text-gray-400">
              {asString(entity.kind) || "Item"}
            </div>
          )}
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-950 dark:text-gray-100">{asString(entity.title) || asString(entity.name) || "Entity"}</p>
            {asString(entity.subtitle) ? <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{asString(entity.subtitle)}</p> : null}
            {asArray(entity.meta).length ? (
              <div className="mt-3 flex flex-wrap gap-2">
                {asArray(entity.meta).map((meta, metaIndex) => {
                  ...
                  return (
                    <span key={`${title}-${metaIndex}`} className="rounded-full bg-white px-2.5 py-1 text-xs text-gray-600 dark:bg-gray-900 dark:text-gray-300">
  ...
  return (
    <WidgetCard key={`${type}-${index}`} title={title || "Unsupported widget"} subtitle={subtitle}>
      <pre className="overflow-x-auto rounded-[20px] bg-gray-50 p-3 text-xs text-gray-700 dark:bg-gray-950 dark:text-gray-300">
  ...
  return (
    <div className="space-y-4">
      {summary || explanation || insights.length ? (
        <WidgetCard>
          <div className="space-y-3">
            {summary ? <p className="text-sm font-semibold leading-6 text-gray-950 dark:text-gray-100">{summary}</p> : null}
            {explanation ? <p className="text-sm leading-6 text-gray-600 dark:text-gray-300">{explanation}</p> : null}
            {insights.length ? (
              <div className="grid gap-3 md:grid-cols-2">
                {insights.map((insight, index) => (
                  <div key={`insight-${index}`} className="rounded-[20px] bg-gray-50 px-4 py-3 dark:bg-gray-950">
          ...
          <div className="flex flex-wrap gap-2">
            {suggestedActions.map((action, index) => (
              <button
                key={`suggested-${index}`}
                type="button"
                className="rounded-full border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800"
                onClick={() =>
                  onSend(
                    JSON.stringify(
