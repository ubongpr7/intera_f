...
function InsightActionForm({
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        className="inline-flex items-center rounded-full bg-gray-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-gray-700"
      >
            ...
            <div key={`${title}-${itemIndex}`} className="flex items-start justify-between gap-3 rounded-[20px] bg-gray-50 px-4 py-3">
              <div className="flex min-w-0 gap-3">
                ...
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-gray-950">{asString(item.label) || asString(item.title) || "Item"}</p>
                  {asString(item.detail) ? <p className="mt-1 text-xs text-gray-500">{asString(item.detail)}</p> : null}
                  {barcodeValue ? (
                    <div className="mt-2">
            ...
            return (
              <div key={`${title}-${itemIndex}`} className="rounded-[20px] border border-gray-200 bg-gray-50 px-4 py-3">
                <div className="flex items-start gap-2">
                  {icon}
                  <div>
                    <p className="text-sm font-semibold text-gray-950">{asString(item.label) || "Risk"}</p>
          ...
          {events.map((event, eventIndex) => (
            <div key={`${title}-${eventIndex}`} className="grid grid-cols-[20px,1fr] gap-3">
              ...
              <div className="rounded-[20px] bg-gray-50 px-4 py-3">
                ...
                <p className="mt-2 text-sm font-semibold text-gray-950">{asString(event.title) || asString(event.event_name) || "Event"}</p>
          ...
          {steps.map((step, stepIndex) => {
            ...
            return (
              <div key={`${title}-${stepIndex}`} className="flex items-start gap-3 rounded-[20px] bg-gray-50 px-4 py-3">
                {icon}
                <div>
                  <p className="text-sm font-semibold text-gray-950">{asString(step.label) || `Step ${stepIndex + 1}`}</p>
    ...
    return (
      <WidgetCard key={`${type}-${index}`} title={title} subtitle={subtitle}>
        <div className="rounded-[20px] bg-gray-50 px-4 py-4">
          <p className="text-sm font-semibold text-gray-950">{asString(widget.summary) || "Confirm this action."}</p>
    ...
    return (
      <WidgetCard key={`${type}-${index}`} title={title} subtitle={subtitle}>
        <div className="flex items-start gap-4 rounded-[20px] bg-gray-50 px-4 py-4">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-gray-950">{asString(entity.title) || asString(entity.name) || "Entity"}</p>
  ...
  return (
    <div className="space-y-4">
      {summary || explanation || insights.length ? (
        <WidgetCard>
          <div className="space-y-3">
            {summary ? <p className="text-sm font-semibold leading-6 text-gray-950">{summary}</p> : null}
