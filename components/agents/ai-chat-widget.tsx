...
export default function AIChatWidget() {
 ...
 const toggleChat = () => {
   setIsOpen((prev) => !prev)
   if (!isOpen) setLastActivityAt(Date.now())
 ...
 useEffect(() => {
   ...
   const poll = async () => {
     ...
     try {
       ...
       if (!cancelled) {
         if (count !== prevSnapshotRef.current.pending) changeDetected = true
     ...
     try {
       ...
       if (!cancelled) {
         if (filtered.length !== prevSnapshotRef.current.events) changeDetected = true
     ...
     try {
       ...
       if (!cancelled) {
         if (filtered.length !== prevSnapshotRef.current.tasks) changeDetected = true
     ...
     if (!cancelled) {
       ...
       if (changeDetected) setLastActivityAt(Date.now())
     }
   }
 ...
 }, [isOpen, sessionId, listMessages, pendingMessages, getEvents, listTasks, messagesMap.size, pendingCount, eventCount, taskCount])

 // Inactivity auto-close after 3 minutes with no work
 useEffect(() => {
   if (!isOpen) return
   ...
   const ticker = setInterval(() => {
     ...
     if (!hasWork && idleFor >= INACTIVITY_MS) {
       setIsOpen(false)
       toast.success("AI Assistant closed due to inactivity.")
     }
   }, 10000)
 ...
 const chatWindowClasses = isFullScreen
   ...
   : "absolute bottom-20 max-w-[450px] right-0 h-[500px] rounded-xl border border-gray-200"
