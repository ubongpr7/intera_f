...
import { useGetLoggedInUserQuery, useUpdateLoggedInUserMutation } from "@/redux/features/users/userApiSlice"
...
const buildInitials = (firstName?: string, lastName?: string, email?: string) => {
  const parts = [firstName, lastName].filter(Boolean).map((part) => String(part).trim())
  if (parts.length) {
    return parts
      .slice(0, 2)
...
export default function UserSettingsPage() {
  ...
  const [updateLoggedInUser, { isLoading: isSavingProfile }] = useUpdateLoggedInUserMutation()
  const [setPassword, { isLoading: isSavingPassword }] = useSetPasswordMutation()
  ...
  const displayName = [user?.first_name, user?.last_name].filter(Boolean).join(" ").trim() || user?.email || "User"
  ...
  const handleSaveProfile = async () => {
    ...
    if (profileForm.date_of_birth) body.append("date_of_birth", profileForm.date_of_birth)
    ...
    try {
      ...
      toast.error(error?.data?.detail || "Unable to update your account settings.")
    }
  }
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-4">
              <Avatar className="h-20 w-20 border border-gray-200">
                ...
                <AvatarFallback className="bg-slate-900 text-xl font-semibold text-white">{initials}</AvatarFallback>
              </Avatar>
              <div>
                <CardTitle className="text-3xl font-semibold tracking-tight text-gray-900">User settings</CardTitle>
                <CardDescription className="mt-2 max-w-2xl text-sm leading-6 text-gray-600">
                  Manage your personal profile, picture, password, and security options here. Workspace and company
                  administration stay under the separate settings hub.
                </CardDescription>
              </div>
            </div>
          ...
          <Card className="border-gray-200 shadow-sm">
            ...
            <CardContent className="space-y-6 p-6 pt-0">
              <div className="flex flex-col gap-5 rounded-3xl border border-gray-200 bg-gray-50 p-5 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-4">
                  <Avatar className="h-16 w-16 border border-gray-200">
                    <AvatarImage src={avatarUrl ?? undefined} alt={displayName} />
                    <AvatarFallback className="bg-slate-900 text-lg font-semibold text-white">{initials}</AvatarFallback>
                  </Avatar>
                  <div>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {avatarFile ? (
                    <Button type="button" variant="ghost" onClick={() => setAvatarFile(null)}>
                      Remove selection
                    </Button>
                  ) : null}
                </div>
              ...
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  <Label>Sex</Label>
                  <Select value={profileForm.sex || undefined} onValueChange={(value) => handleProfileFieldChange("sex", value)}>
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="not_to_mention">Prefer not to mention</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
              ...
              <div className="flex justify-end">
                <Button type="button" onClick={handleSaveProfile} disabled={isLoading || isSavingProfile}>
        ...
        <TabsContent value="security" className="space-y-6">
          <Card className="border-gray-200 shadow-sm">
            <CardHeader className="p-6">
              ...
              <CardDescription>Manage password changes, MFA setup, and account verification from one place.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6 p-6 pt-0">
                  ...
                  <CardHeader className="p-5">
                    ...
                    <CardDescription>
                      {user?.mfa_enabled
                        ...
                        : "Set up MFA to protect account sign-in and verification."}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="flex gap-3 p-5 pt-0">
                    <Button asChild variant="outline">
                      <Link href="/accounts/mfa/setup">Setup MFA</Link>
                    </Button>
                    <Button asChild variant="outline">
                      <Link href="/accounts/mfa/verify">Verify MFA</Link>
                    </Button>
                  </CardContent>
                </Card>
                  ...
                  <CardHeader className="p-5">
                    ...
                    <CardDescription>
                      {user?.is_verified
                        ...
                        : "Complete verification to fully activate your account access."}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="flex gap-3 p-5 pt-0">
                    <Button asChild variant="outline">
                      <Link href="/accounts/verify">Open verification</Link>
                    </Button>
                  </CardContent>
                </Card>
              </div>
