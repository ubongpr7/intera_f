...
export function useSubscriptionQuota(featureSlug: string, usage: number, requested = 1) {
  ...
  else if (!data?.subscription) message = "Choose a subscription plan before creating this resource."
  ...
  return { canCreate, feature, isLoading, limit, message, remaining, unlimited, usage: normalizedUsage }
}
